...
import os
import logging
from typing import AsyncIterator, Dict, Iterator, Optional, Any, Union
...
class AzureOpenAIProvider(LLMProvider):
    ...
    def _should_use_v1_endpoint(self, model: str) -> bool:
        ...
        v1_prefixes = [
            ...
            "gpt-5-mini", "gpt-5-nano", "gpt-5-pro", "gpt-5-codex",
        ]
        ...
        if request.tools and capabilities.supports_tools:
            ...
            if request.parallel_tool_calls is not None:
                if capabilities.supports_parallel_tool_calls:
                    ...
                    logger.debug(
                        "Model does not support parallel_tool_calls, ignoring",
                        model=request.model
                    )
    ...
    def _prepare_messages(
        ...
        for msg in request.messages:
            ...
            if (capabilities.convert_system_to_developer and 
                msg_dict.get("role") == "system"):
                
                if capabilities.supports_developer_message:
                    msg_dict["role"] = "developer"
                    logger.debug(
                        "Converted system message to developer message",
                        model=request.model
                    )
    ...
    def stream(self, request: CompletionRequest) -> Iterator[CompletionChunk]:
        ...
        if not capabilities.supports_streaming:
            logger.warning(
                "Model does not support streaming, falling back to generate()",
                model=request.model
            )
    ...
    async def astream(self, request: CompletionRequest) -> AsyncIterator[CompletionChunk]:
        ...
        if not capabilities.supports_streaming:
            logger.warning(
                "Model does not support streaming, falling back to agenerate()",
                model=request.model
            )
